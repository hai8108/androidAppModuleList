...
public class FrescoConfigs {
    ...
    private static ImagePipelineConfig configureCaches(Context context) {
        ...
        final MemoryCacheParams bitmapCacheParams = new MemoryCacheParams(
                FrescoConfigs.MAX_MEMORY_CACHE_SIZE, // 内存缓存中总图片的最大大小,以字节为单位。
                Integer.MAX_VALUE,                     // 内存缓存中图片的最大数量。
                FrescoConfigs.MAX_MEMORY_CACHE_SIZE, // 内存缓存中准备清除但尚未被删除的总图片的最大大小,以字节为单位。
...
//    		  .setDiskTrimmableRegistry(diskTrimmableRegistry)//类将包含一个注册表的缓存减少磁盘空间的环境。
                .setMaxCacheSize(FrescoConfigs.MAX_DISK_CACHE_SIZE)//默认缓存的最大大小。
...
//			  .setFadeDuration(fadeDuration)//fresco:fadeDuration="300"加载图片动画时间
                .setFailureImage(FrescoConfigs.sErrorDrawable)//fresco:failureImage="@drawable/error"失败图
...
//			  .setOverlays(overlays)
                .setPlaceholderImage(FrescoConfigs.sPlaceholderDrawable)//fresco:placeholderImage="@color/wait_color"占位图
